    ...
    <div className="cost-time-block">
      {summaries.map((s) => (
        <div key={s.resourceType} className="cost-time-block__row">
          ...
          {s.time > 0 && <span className="cost-time-block__time">{formatTime(s.time)}</span>}
        </div>
      ))}
    </div>
  );
...
function TrapRow({
  id, lvl, name, upgrade, thLevel, goldPassMultiplier = 1,
}: { id: number; lvl: number; name: string; upgrade?: UpgradeEntry[]; thLevel: number; goldPassMultiplier?: number; }) {
...
function TrapGroup({
  id, instances, thLevel, goldPassMultiplier = 1, onGroupTotal,
}: {
  id: number; instances: number[]; thLevel: number; goldPassMultiplier?: number;
  onGroupTotal?: (id: number, costs: CostTime[]) => void;
}) {
  ...
  const allAtMax = upgrade != null && instances.every(
    (lvl) => getLevelStatus(lvl, maxLevelForGroup, absoluteMaxForGroup) !== 'normal',
  );
  ...
  return (
    <div className="bld-group">
      ...
      {instances.map((lvl, i) => (
        <TrapRow key={i} id={id} lvl={lvl} name={name} upgrade={upgrade} thLevel={thLevel} goldPassMultiplier={goldPassMultiplier} />
      ))}
    </div>
  );
}
...
export function TrapList({ traps, thLevel, goldPassMultiplier = 1 }: Props) {
  const [groupTotals, setGroupTotals] = useState<Map<number, CostTime[]>>(new Map());

  const reportGroupTotal = useCallback((id: number, costs: CostTime[]) => {
    setGroupTotals((prev) => {
  ...
  return (
    <div className="bld-list">
      ...
      {[...grouped.entries()]
        .sort((a, b) => a[0] - b[0])
        .map(([id, instances]) => (
          <TrapGroup key={id} id={id} instances={instances} thLevel={thLevel} goldPassMultiplier={goldPassMultiplier} onGroupTotal={reportGroupTotal} />
        ))}
    </div>
  );
}
